...
OXCはメモリ効率でもBiomeを上回ります。CI時間の削減は、開発者の待ち時間短縮だけでなくGitHub Actionsの課金額削減にも直結します。
...
OXCの公式VS Code拡張「oxc」をインストールすると、エディタ上でリアルタイムにlint結果が表示されます。ESLint拡張と併用する場合は、同じルールが重複しないようどちらか一方を無効化しましょう。
